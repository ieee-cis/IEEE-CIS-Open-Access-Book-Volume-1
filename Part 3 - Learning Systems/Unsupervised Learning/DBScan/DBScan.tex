...
%\Statex
\STATE{$C \gets 0$}
\FOR {all unvisited point $\mathbf{x}$ in $\mathcal{X}$}
\STATE mark $\mathbf{x}$ as $Visited$
\STATE $N_{\epsilon}(\mathbf{x})$ $\gets$ regionQuery($\mathbf{x}$, $\epsilon$)  \algorithmiccomment{finding initial neighbourpoints of $\mathbf{x}$}
...
\ELSE
    ...
    \STATE {expandCluster($\mathbf{x}$, NeighborPoints, C, $\epsilon$, $MinPts$) \COMMENT{See Algorithm \ref{alg:expandCluster}.}}
\ENDIF
\ENDFOR
\end{algorithmic}
\end{algorithm}
...
\FOR {all point $\mathbf{x}'$ in $N_{\epsilon}(\mathbf{x})$}
    \IF {$\mathbf{x'}$ is not $visited$}
        \STATE {mark $\mathbf{x}'$  as $visited$}
        \STATE {$N_{\epsilon}(\mathbf{x'})$ $\gets$ regionQuery($\mathbf{x}'$ , $\epsilon$) \algorithmiccomment{See Algorithm \ref{alg:regionQuery}.}}
    \ENDIF
...
\item The code of solution can be found at 

\url{https://github.com/ieee-cis/IEEE-CIS-Open-Access-Book-Volume-1/blob/main/Part%203%20-%20Learning%20Systems/Unsupervised%20Learning/DBScan/code/find_eps.py} 

or 

\url{https://gist.github.com/LinaYao/00f3b2cf65600f7b03df020c81cc5de9}.
\end{enumerate} 
