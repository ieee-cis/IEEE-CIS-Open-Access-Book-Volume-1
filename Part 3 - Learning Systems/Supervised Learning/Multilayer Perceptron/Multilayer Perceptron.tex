...
\begin{figure}[h!]
	...
	\vspace{0.2cm}
	\begin{center}
		\includegraphics[width=0.8\textwidth]{neuronioArtificial_v2@4x.png}
	\end{center}
	\label{fig:neuronio}
\end{figure}

This figure shows that the input signals $x_i , i=1, 2,..., I$ and a fixed value bias signal $+1$ are weighted by the weights $w_{ji}$ in each neuron $j$. The activation $u_j$ (induced local field) is reached with these terms, which goes through the activation function $f(\cdot)$, generating the output $\hat{y}_j$. In general, the activation function $f(\cdot)$ is nonlinear. The mathematical representation of this model is given by Equation \ref{eq:neuronio}, where the index $i=0$ represents the bias:
...
\noindent
If $u$ is greater than zero, the output will equal the input.
...
\begin{figure}[h!]
    ...
    \vspace{0.2cm}
    \includegraphics[width=1\textwidth]{MLP_v2@4x.png}
    \label{fig:mlp}
\end{figure}
...
Consider as training data ($\mathbf{x}(n), \mathbf{y}(n)$) for iteration $n$, where $\mathbf{x}(n)$ is the input vector applied to the ANN input layer, and $\mathbf{y}(n)$ is the desired response vector. The input signal propagation is done by calculating the $u_j^{(l)}(n)$ induced local fields for neuron $j$ in layer $l$ (Equation \ref{eq:backprop1}):
