...
\end{equation}
where $\hat{y}_i^{(l-1)}(n)$ is an input of neuron $j$ of layer $l$ corresponding to:
\begin{itemize}
\item the input feature $x_i$ of the problem being solved, if this is the first hidden layer,
\item the output signal of neuron $i$ of the previous layer $l-1$, if this is not the first hidden layer, 
\item the bias value +1 if $i=0$). 
\end{itemize}
The value $w_{ji}^{(l)}(n)$ is the weight of the neuron $j$ in layer $l$ that is connected to neuron $i$ in layer $l-1$.
