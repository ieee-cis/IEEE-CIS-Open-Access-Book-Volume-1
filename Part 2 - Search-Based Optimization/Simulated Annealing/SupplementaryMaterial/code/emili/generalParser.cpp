...
#define DEFAULT_IT 0
#define GIT_COMMIT_NUMBER "faed3fb21359ee09b660ab99c1d433ec96978196"
/*Base Initials*/
...
{
    tm++;
    tm++; // Skipping first two arguments ( executable name and instance path)
    char* p = tm.peek(); // Load problem keyword
    if(p != nullptr)
    {
    std::string prob(p);
    Builder* probBuilder = nullptr; // Builder used to load the problem instance
    // Looking for a Builder to load problem instance and
    for(std::vector< Builder*> ::iterator iter= allbuilders.begin(); iter!=allbuilders.end(); ++iter)
    {
        Builder* bld = *iter;
        if(bld->isCompatibleWith(prob)) // If bld can build components for prob
        {
           activeBuilders.push_back(bld); // Add bld to the active builders
           if(bld->canOpenInstance(prob)) // If bld can load prob instances
           {
               probBuilder = bld;
           }
        }
    }
    if(probBuilder != nullptr) // If there is a builder that can load the problem instance
    {
        emili::initializeRandom(getSeed(tm));
        this->instance = probBuilder->openInstance(); // Load problem instance
        prs::Component cls = buildComponent(COMPONENT_ALGORITHM); // Load an algorithm
        if(cls.getType() == COMPONENT_EMPTY || cls.getType() == COMPONENT_NULL) // Check if the algorithm has been loaded correctly
        {
            fatalError(cls.getType(),COMPONENT_ALGORITHM);
        }
        emili::LocalSearch* ls = cls.get<emili::LocalSearch>();
        ls->setSearchTime(getTime(tm,ls->getInitialSolution().getProblem().problemSize())); // Setup local search
        emili::setRootAlgorithm(ls); // Set root algorithm
        initializeTimerBaseSolution(ls->getInitialSolution().generateEmptySolution());// Initialization of the solution pointer used by finalise to store the best solution when the timer is called.
        if(tm.seek(PRINT_SOLUTION)>0) // If the print option is enabled
        {
            emili::set_print(true);            
            if(ls->getSearchTime() > 0){
                std::cout << "The solution will be print at the end of the execution" << std::endl;
