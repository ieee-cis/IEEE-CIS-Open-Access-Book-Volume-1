...
emili::Solution* emili::StatsBISearch::search() {
    emili::Solution* current = init->generateSolution();
    emili::Solution* sol = emili::StatsBISearch::search(current);
    if(current!=sol)
        delete current;
...
emili::Solution* emili::StatsBISearch::search(emili::Solution* initial)
{
        termcriterion->reset();
        neighbh->reset();
